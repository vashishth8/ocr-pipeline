            ...
            pages.append(
                {
                    ...
                    "poppler_display_bbox": [
                        float(pymupdf_page.rect.x0),
                        float(pymupdf_page.rect.y0),
                        float(pymupdf_page.rect.x1),
                        float(pymupdf_page.rect.y1),
                    ],
        ...
        out.append(
            {
                ...
                "words": ws,
                "text": block_text(ws),
                "bbox": bbox_union([[w["x0"], w["y0"], w["x1"], w["y1"]] for w in ws]),
            }
        )
    return out
...
def build_structure_page(page, repeated_headers):
    lines = merge_heading_fragments(build_visual_lines(page["words"]))
    ...
    for i, line in enumerate(lines):
        ...
        if text.lower() in repeated_headers and y0 <= page_top + page["height"] * 0.15:
            header_idxs.add(i)

        if is_page_number(text) and y1 >= page_bottom - page["height"] * 0.10:
            footer_idxs.add(i)
    ...
    while i < len(lines):
        ...
        if is_caption(text):
            blocks.append(
                {
                    "type": (
                        "table_caption" if text.lower().startswith("table") else "figure_caption"
                    ),
                    "text": text,
                    "bbox": line["bbox"],
                }
            ...
            blocks.append(
                {
                    ...
                    "bbox": bbox_union([[w["x0"], w["y0"], w["x1"], w["y1"]] for w in all_words]),
                }
            )
            i = j
            continue
    ...
    for block in blocks:
        if block["type"] == "paragraph":
            ...
            if (
                len(t) <= 3
                and page_top + page["height"] * 0.18
                < block["bbox"][1]
                < page_top + page["height"] * 0.88
                and h > 0
            ):
                continue
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "pdf", type=Path, help="source PDF; it is never copied into the output directory"
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
...
def main(argv: list[str] | None = None) -> int:
    ...
    structure_pages = [build_structure_page(page, headers) for page in pages]
    ...
    for p in structure_pages:
        print(f"  Page {p['page']}: {len(p['blocks'])} blocks {p['structure_summary']}")
    return 0
