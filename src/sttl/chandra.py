...
from sttl.text import html_to_text as _html_to_text

API_URL = "https://www.datalab.to/api/v1/convert"
TERMINAL_STATUSES = {"complete", "failed", "error"}
RESULT_DOWNLOAD_TIMEOUT = (30, 180)
MISSING_PAGE_SORT_KEY = 10**9


def html_to_text(value: Any) -> str:
    """Preserve Chandra's historical Unicode-aware HTML text normalization."""
    return _html_to_text(value, collapse_unicode_whitespace=True)
...
def require_api_key() -> str:
    ...
    if not key:
        raise SystemExit("DATALAB_API_KEY is not set. Set it in the environment before rerunning.")
    return key
...
def has_inline_result(result: dict[str, Any]) -> bool:
    """Return whether a completed status response already contains its output.

    The Convert result endpoint can return the result inline or provide a
    short-lived ``result_url``.  An explicitly present empty inline value is
    still an inline result: a blank document must not trigger an unnecessary
    signed-URL download.
    """
    return any(result.get(key) is not None for key in ("json", "markdown", "html", "chunks"))


def validated_result_url(value: Any) -> str:
    """Validate a result URL without ever echoing its signed query string."""
    if not isinstance(value, str) or not value.strip():
        raise RuntimeError(
            "Completed Chandra result has an invalid result_url; expected an "
            "absolute HTTPS URL. Re-poll the request to obtain a fresh result URL."
        )
    ...
    if (
        ...
        or any(character.isspace() for character in result_url)
    ):
        raise RuntimeError(
            "Completed Chandra result has an invalid result_url; expected an "
            "absolute HTTPS URL. Re-poll the request to obtain a fresh result URL."
...
def download_result_url(
    session: requests.Session,
    result_url: Any,
) -> dict[str, Any]:
    """Download a completed JSON result from its signed URL.

    Signed URLs contain their own authorization, so deliberately omit the API
    key header.  They are accepted only over HTTPS and redirects are disabled
    so a result URL cannot silently downgrade transport or forward its signed
    query string to another host.
    """
    url = validated_result_url(result_url)
    try:
        ...
        raise RuntimeError(
            "Could not download the completed Chandra result from its signed "
            "result_url. It may have expired; re-poll or rerun the conversion."
        ) from error

    if isinstance(response.status_code, int) and 300 <= response.status_code < 400:
        raise RuntimeError(
            "Refused a redirect while downloading the completed Chandra result "
            "from its signed result_url. Re-poll the request for a direct, fresh URL."
        )
    ...
    try:
        ...
        raise RuntimeError(
            "The completed Chandra result download was not valid JSON. Its "
            "signed result_url may have expired or returned an unexpected response."
        ) from error

    if not isinstance(payload, dict):
        raise RuntimeError(
            "The completed Chandra result download must be a JSON object; "
            f"received {type(payload).__name__} instead."
        )
    return payload


def resolve_completed_result(
    session: requests.Session,
    result: dict[str, Any],
) -> dict[str, Any]:
    """Resolve an optional signed result URL after a successful terminal poll.

    Inline API results retain their established behavior.  Only a ``complete``
    response that has no inline output is eligible for the extra download.
    """
    if str(result.get("status", "")).lower() != "complete" or has_inline_result(result):
        return result
    ...
    downloaded = download_result_url(session, result_url)
    # Keep completion metadata as a fallback while allowing the downloaded
    # result to provide the OCR payload and any richer result fields.  The
    # terminal polling state remains authoritative for the caller's status
    # check, even if the downloaded JSON omits it.
    resolved = {**result, **downloaded}
    resolved["status"] = result.get("status", "complete")
    return resolved
...
def poll_result(
    ...
    while True:
        if time.perf_counter() - started > timeout_seconds:
            raise TimeoutError(f"Timed out after {timeout_seconds:.0f}s while polling {check_url}")
        ...
        if status in TERMINAL_STATUSES:
            elapsed = time.perf_counter() - started
            # Datalab may return the completed payload inline or supply a
            # short-lived signed result_url.  Resolve the latter only after
            # this terminal response has been received.
            return resolve_completed_result(session, result), elapsed, polls
...
def collect_page_like_nodes(obj: Any) -> list[dict[str, Any]]:
    ...
    pages = [
        node for node in children if isinstance(node, dict) and node.get("block_type") == "Page"
    ]
...
def normalize_chandra(result: dict[str, Any]) -> list[dict[str, Any]]:
    """Convert provider-shaped OCR fields into page-oriented evidence.

    Structured pages take precedence over markdown so labels and block order
    survive normalization. Any retained coordinates remain provider-native;
    they must not be treated as STTL's normalized PDF-point geometry.
    """
    ...
    if pages:
        ...
        pages_sorted = sorted(
            pages,
            key=lambda p: (
                p.get("page", p.get("page_number", MISSING_PAGE_SORT_KEY))
                if isinstance(
                    p.get("page", p.get("page_number", MISSING_PAGE_SORT_KEY)), (int, float)
                )
                else MISSING_PAGE_SORT_KEY
            ),
        )
...
def load_saved_response(response_path: Path) -> dict[str, Any]:
    ...
    if not isinstance(result, dict):
        raise ValueError(f"Saved response has a non-object 'result' field: {response_path}")
    return result
...
def normalize_saved_response(
    ...
    output_path, normalized = write_normalized_output(output_dir, stem, source_name, result)

    if write_markdown and isinstance(result.get("markdown"), str):
        (output_dir / f"{stem}_chandra.md").write_text(result["markdown"], encoding="utf-8")
...
def main() -> int:
    ...
    if status != "complete" or not result.get("success", False):
        print(
            json.dumps({"timing": timing, "error": result.get("error")}, indent=2), file=sys.stderr
        )
        return 1
